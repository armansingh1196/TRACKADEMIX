...
const TeacherProfile = () => {
    ...
    const notifKey = `notifPrefs_teacher_${currentUser?._id}`;
    ...
    /* Notifications */
    const defaultNotifs = { attendance: true, exams: true, notices: true };
    ...
    const NOTIF_OPTIONS = [
        { key: 'attendance', label: 'Class Attendance Reminders', desc: 'Remind before class sessions start' },
        { key: 'exams',      label: 'Exam Schedule Alerts',       desc: 'Upcoming evaluation notifications' },
    ...
    return (
        <Container maxWidth="md" sx={{ mt: 2, mb: 6 }}>
            <AppHeader title="Faculty Profile" subtitle="Manage your credentials, security, and preferences." />

            {/* ── Identity ── */}
            <SectionCard>
                <SectionLabel><PersonOutlineRoundedIcon sx={{ fontSize: 13 }} />Faculty Identity</SectionLabel>
                <Grid container spacing={4} alignItems="flex-start">
                    <Grid item xs={12} md={3} sx={{ display:'flex',flexDirection:'column',alignItems:'center',gap:1.5 }}>
                        ...
                        <AvatarRole>Faculty Professor</AvatarRole>
                    ...
                    <Grid item xs={12} md={9}>
                        <FieldGrid>
                            ...
                            <FieldBlock>
                                <FieldLabel>Assigned Subject</FieldLabel>
                                <FieldValue><AssignmentIndOutlinedIcon sx={{ fontSize:16,color:'rgba(124,77,255,0.7)' }} />{teachSubject?.subName || '—'}</FieldValue>
                            </FieldBlock>
                            <FieldBlock>
                                <FieldLabel>Target Class</FieldLabel>
                                <FieldValue><SchoolOutlinedIcon sx={{ fontSize:16,color:'rgba(124,77,255,0.7)' }} />{teachSclass?.sclassName || '—'}</FieldValue>
                            </FieldBlock>
                            <FieldBlock style={{ gridColumn:'1 / -1' }}>
                                <FieldLabel>Institution</FieldLabel>
                                <FieldValue><BusinessOutlinedIcon sx={{ fontSize:16,color:'rgba(124,77,255,0.7)' }} />{teachSchool?.schoolName || '—'}</FieldValue>
                            </FieldBlock>
                        </FieldGrid>
                        <VerifiedBadge>
            ...
            <SectionCard>
                ...
                <NotifDesc>Control which alerts you receive. Preferences are saved locally on this device.</NotifDesc>
                <NotifList>
...
const SectionCard = styled.div`
    background:rgba(255,255,255,0.03);border:1px solid rgba(124,77,255,0.1);border-radius:20px;padding:28px 32px;
    margin-bottom:20px;backdrop-filter:blur(40px);animation:${fadeUp} 0.5s cubic-bezier(0.16,1,0.3,1) both;
